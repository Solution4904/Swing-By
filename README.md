...
- baseActivity
- KakaoAPI latLng값들 Double로 하나씩 받아오고 있는 것들 latlng으로 바꿀것 (+ 이런식으로 두개 이상의 값이 들어있는 변수를 어떻게 생성해서 사용하는지 타고가서 볼 것)
- 메모 데이터에 chip 그룹을 카카오맵 검색 카테고리랑 일치화 시키고 검색할 때 이걸 사용해서 정확도를 높일 것
- 검색 결과 지도 화면에서 현재 위치도 표시시켜야 함